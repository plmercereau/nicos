...
- bastions
  ...
  - the bastions also run a `dnsmasq` server on port 53 that listens to the Wireguard network interface, with the list of the hosts and their IP addresses stored in `/etc/hosts`

## Configure a peer

Generate a wireguard private key and a public key for the `client` machine:

```bash
nix run github:plmercereau/nicos -- secret vpn client
```

It encrypted the private key to `hosts-nixos/client.vpn.age`.

It also printed the corresponding public key. Don't forget to paste it to `settings.networking.vpn.publicKey` in `hosts-nixos/client.nix`.
...
A bastion first needs to be configured in the same way as any other machine: it needs to have a public key, a private key, and a unique id, that is prepared in the same way as explained in the above section.

Then, enable the machine as a VPN bastion by setting `settings.networking.vpn.bastion.enable` to `true`.
