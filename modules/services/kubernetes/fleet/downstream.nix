...
with lib; let
  ...
  script = pkgs.writeScript "send-fleet-kubeconfig" ''
    FLAG_FILE="/var/lib/nicos/fleet.ok"
    if [[ -e "$FLAG_FILE" ]]; then
        echo "Script has already run. Exiting."
        exit 0
    fi
    ${pkgs.openssh}/bin/ssh -i /etc/ssh/ssh_host_ed25519_key ${cfg.connectionUser}@${upstreamMachine.lib.vpn.ip} "$(cat /etc/rancher/k3s/k3s.yaml)"
