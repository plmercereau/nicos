inputs @ {
  ...
  fromTemplate = template: contents: let
    template_file = builtins.readFile (./. + "/${docPath}/${template}");
  in
      ...
      lib.mapAttrsToList (
        name: value: ''
          ...
          Declared in ${builtins.concatStringsSep ", " (builtins.map (d: let
            file = lib.removePrefix "${toString ./.}/" (d.file);
...
in
  pkgs.writeShellApplication {
    ...
    text = ''
      umask 022
      mkdir -p ${docPath}/reference/machines
      cp -f ${commonFile} ${docPath}/reference/machines/common.mdx
      cp -f ${nixosFile} ${docPath}/reference/machines/nixos.mdx
      cp -f ${darwinFile} ${docPath}/reference/machines/darwin.mdx
      cp -f ${hardwareFile} ${docPath}/reference/hardware.mdx
      awk '/%CONTENT%/{system("${cli}/bin/nicos docgen --bin-cmd \"${cliBin}\"");next}1' ${cliTemplate} > ${docPath}/reference/cli.mdx
    '';
  }
